...
- `excel_check_record`：开放已用真实任务单证明的两条数据链：
  `5103.5 / 纤维微观形貌`（26A045793）与 `5103.426 / 纤维横截面`
  （260191285），测试方法均为 `GB/T 36422-2018`。每个包只含一个 `.xls`，
  只允许一个关键结果。schema v1 继续只接受 `微观形貌.xls`，且
  `expected_key_identities` 必须是“纵面”或“横截面”；schema v2 接受下表
  列出的精确模板及其映射指纹，关键结果身份允许为空。
...
覆盖任务包字段、单引号、工作簿路径/长度/SHA-256、v1 兼容、十模板精确映射（微观形貌
七个、横截面三个）、横截面项目三元组、v2

...
### 登记工作簿格式保留契约（2026-08-13）

后端 BIFF 补丁器（`backend/app/execution/biff_patch.py`）只允许改动声明单元格的文本，
字体、填充色、边框、公式和其它流字节必须保持模板原状。注意这些模板把 Z7、I9–I11、
G12、G13、BI8 等空格存放在 **MULBLANK 区间**中：写入被区间覆盖的单元格时必须围绕目标列
拆分区间，并让新记录继承区间分配给该列的 XF（如 Z7=79、I9/G12 系=74、G13=75、
BI8=72）。在区间之外插入带外来 XF 的重复记录会让 Excel 用后一条记录覆盖模板底色/边框
（2026-08-13 Z7 实测回归）。MULBLANK 的 `colLast` 是载荷末尾的**两字节**字段；
MULRK 数值区间的写入显式拒绝。范围校验器按“同记录 id 配对 + 允许插入
LABELSST/STRING/MULBLANK/BLANK”放行拆分产物，数值类记录仍禁止插入。

...
- 修复：生成时经 UNO 把 K2 显式改写为中文日期格式 `YYYY"年"M"月"D"日"`
  （模板二进制与 SHA 绑定不动）；TODAY() 公式保留，日期仍随打开/打印日
  刷新。写入器保存后重开文件，按“渲染显示串必须匹配
  `^\d{4}年\d{1,2}月\d{1,2}日$`”做行为验证（LibreOffice 会把格式码
  规范化为反斜杠转义形式，故不比较格式码字面量）；调用方
  `_verify_generated_workbook` 新增 `number_format_verified` 强制检查，
  不达标则整批生成失败。
- 验证：execution-worker 容器内真实 LibreOffice 端到端跑通，产物 K2
  持久化为 `yyyy\年m\月d\日`、值仍为当日（TODAY 语义不变）；宿主新增
  payload 约定测试 1 项。

...
- 问题：生成原始记录右上角日期在中文 Windows 上显示为美国顺序
  （如 8/15/2026）。根因：模板 K2 是 TODAY() 公式，使用 BIFF 内建
  格式 0x0E（m/d/yy，随系统区域渲染），LibreOffice 往返保存将其固化
  为 en-US 显式格式 `[$-409]m/d/yyyy`，从此与查看环境区域脱钩。
- 修复：生成时经 UNO 把 K2 显式改写为中文顺序日期格式 `YYYY/M/D`
  （首版曾用 `YYYY"年"M"月"D"日"`，按用户要求调整为斜杠式；模板二进制
  与 SHA 绑定不动）；TODAY() 公式保留，日期仍随打开/打印日刷新。
  写入器保存后重开文件，按“渲染显示串必须匹配 `^\d{4}/\d{1,2}/\d{1,2}$`”
  做行为验证（不比较格式码字面量，LibreOffice 可能规范化写法）；调用方
  `_verify_generated_workbook` 新增 `number_format_verified` 强制检查，
  不达标则整批生成失败。
- 验证：execution-worker 容器内真实 LibreOffice 端到端跑通，产物 K2
  按 `yyyy/m/d` 持久化、值仍为当日（TODAY 语义不变）；宿主新增
  payload 约定测试 1 项。

## 2026-08-19：图片放置崩溃残留接管与断点续放

- 修复审阅发现的两个崩溃窗口恢复体验问题：
  1. 无覆盖自动放置若在“预留占位”后、原子提交前崩溃，零字节预留会在
     重试时被当成同名冲突，产生莫名的人工确认（幻影冲突）；
  2. 已持久化的覆盖授权与当前计划不匹配时直接转人工，即使目标内容
     其实已完整落盘，也不会走内容识别。
- 预留文件改为写入自描述标记（`textile-report-image-placement-reservation:`
  + 32 位十六进制），计划阶段不再把标记残留列为冲突，执行阶段可安全
  接管（删除标记后排他重建）；零字节等外来占位文件仍严格算冲突。
- 内容识别升级为统一现状分析：全部目标内容一致 → 直接收尾；已存在
  目标全部一致、其余缺失或为标记残留 → 以无覆盖语义补齐缺失部分
  （`resumed_prior_attempt`）；存在内容不一致的外来文件 → 照旧转人工。
  覆盖授权不匹配时撤销授权后同样先走该分析，已完成批次零打扰收尾。
- 识别路径改为流式直读源文件算哈希（不再为校验先写临时文件）；执行
  入口按需清扫超过一小时的本系统隐藏临时文件与标记残留（远离租约
  时长，不误伤在途尝试）。
- 测试：新增 6 项（残留不算冲突/外来占位仍冲突/残留接管/断点续放/
  deferred mismatch 识别/陈旧临时文件清扫），专项 31 项通过。

## 2026-08-18：电镜登记与报告图片放置可靠性修复

- 检验记录生成器升级至 v4：AS4 仅对不含前导零且不超过 15 位的纯数字
  编号使用数值格，其他编号按文本精确写入；生成器版本进入请求摘要，
  新旧生成契约使用不同暂存路径，安全的 v3 在途制品仍可继续登记。
- 报告图片放置计划保留索引 ID 与源指纹，复制前后均核对源文件；整批文件
  先暂存再提交，无覆盖模式先排他预留，覆盖模式保留回滚副本。提交失败
  自动恢复，回滚失败则保留恢复备份并转人工核对，不再产生静默截断或
  部分成功。人工确认后出现的新冲突不会继承旧授权，必须重新确认。
- 人工任务提交事务只保存覆盖决定并把节点重新排队；Worker 在 CIFS 遍历
  与复制前提交并释放数据库事务，完成后再以短事务落回执。完全落盘但
  数据库完成失败的重试可按内容识别已完成批次。
- 部署示例补齐 `SMB_USER_C/SMB_PASS_C`；报告图片共享根纳入实际可写探测、
  工作流可用性与 `/health/ready`。
- 验证：执行系统专项 169 项及 64 个子测试通过；后端全量 599 项通过、
  4 项跳过，唯一失败为本机缺少 UNO 的既有面积法用例；前端 Node 18 项与
  Vitest 120 项通过；`docker compose config --quiet` 与 `git diff --check`
  通过。尚未完成真实 CIFS、UNO、PostgreSQL 或旧系统写入验收。

## 2026-08-18：检验编号字母兼容修复 + 电镜流程恢复打印确认节点

- 修复“生成检验记录登记工作簿”对字母检验编号（第三位可为字母，如
  26A045793）崩溃的问题：登记模板 AS4（样品编号）原是文本占位格
  （“由系统写入的报告号”），代码却一律 `float()` 数值化写入。现纯数字
  且无前导零的编号保持数字格写入（与已验收生产行为一致），含字母或带
  前导零的编号按文本写入；生成后校验同步改为数值/文本双口径比对。
  全面排查结论：后端执行模块与两个 C# Writer 中对检验编号做数值转换的
  仅此一处（特种毛顺号分配、C# 包模型、回读校验均为字符串处理）。
- 电镜微观形貌/横截面流程恢复“确认是否打印原始记录”节点
  （generate-record → print-confirm → upload-record）：a39e6e85 曾将其
  从当前契约移除以贯通自动交付（该节点为纯人工确认，无自动化价值；
  变更记录于 2026-08-13 CHANGELOG）。应用户要求恢复，指引明确为
  “使用默认打印机打印工作表“微观形貌”的默认打印区域”，选择打印需
  确认已在 Excel 完成打印，选择暂不打印则直接继续。
- 定义构建器新增 `legacy_no_print_contract` 开关重建“含图片放置、无打印
  确认”的昨日已发布定义（微观形貌 0a91e1da…/横截面 ee427042…），
  兼容集合登记后存量库自动升级：微观形貌 v6（9dad25e4…）、横截面 v4
  （39ea04db…）；更早时代校验和（df00b313…等）保持不变。前端打印指引
  文案同步补“默认打印机”；既有打印确认 UI 组件直接复用，无需新开发。
- 测试：AS4 字母/前导零编号新增用例；最终验证结论见上方可靠性修复记录。
  本机 dev 库微观形貌因人工发布过 v5（updated_by_id 非空）再次
  被护栏拒升，核对后已手动对齐发布 v6；生产库无此历史，届时自动升级。

## 2026-08-18：电镜流程新增“放置报告上传图片”节点

- 电镜微观形貌与横截面两条流程末尾（检验记录登记完成后）新增
  `place-report-images` 节点：把本次选图按 `{编号}-{样品识别}[-序号]`
  命名写入 `\\192.168.105.82\公共交换文件\数据分析中心\3-报告上传图片\
  8-材料检测中心\1-微观形貌-GB T 36422\{编号}\`（不存在则创建）；
  同名冲突时暂停询问覆盖/取消，询问界面提供可复制完整路径；
  无冲突则零人工干预自动放置。样品识别取流程确认值（与 Z7/登记一致），
  命名规则与共用目录均经用户确认。详见 `15_报告上传图片放置节点.md`。
- 实现：复用 `human.input` + `report_image_placement` 标记（自动完成/
  人工决定双路径），新模块 `report_image_placement.py`；写入用排他创建
  防静默覆盖；回执含 SHA-256。新存储根 `report_upload_images`（write），
  路径分层：节点 config `target_directory` 设计器可改，UNC 展示前缀走
  `EXECUTION_REPORT_IMAGE_DISPLAY_UNC`（流程定义校验禁绝对路径）。
- 挂载：主 compose 新增可写 CIFS 卷 `report_images_cifs`
  （`//192.168.105.82/公共交换文件`），凭据独立变量 `SMB_USER_C/SMB_PASS_C`
  ——该共享拒绝匿名挂载（材料检测中心可访客只读，二者凭据策略不同）；
  Docker CIFS 卷会固化创建时凭据，换凭据须先删卷。本机 local.env 的
  SMB_USER_B/C 已写入真实账号；生产 server.env 模板与部署包已同步补齐。
- 升级：定义构建器加 `legacy_image_placement_contract` 开关重建旧形状，
  两条流程兼容集合登记后启动自动追加版本（微观形貌 v4→v5、横截面
  v2→v3）。微观形貌草稿曾被设计器保存（仅序列化差异），护栏按设计拒升，
  核对无实质改动后已按系统定义对齐并发布 v5。
- 测试：新增 16 项；电镜/横截面/纸类/引擎/校验/持久化回归 137 项，
  外部操作/存储等 209 项，前端 120 项与生产构建全部通过；
  真实共享盘活体冒烟（放置/冲突转人工/UNC 展示）通过。
